...
class XMLToJSONConverter {
    ...
    processNode(node) {
      ...
      if (hasMixedContent) {
        ...
        const innerContent = node.innerHTML || this.nodeProcessor.getInnerHTML(node);
      ...
      if (node.nodeType === this.domEnv.nodeTypes.ELEMENT_NODE && node.hasAttributes()) {
    ...
    processRegularContent(nodeObj, node, context) {
      ...
      if (node.nodeValue) {
        const value = this.nodeProcessor.applyTransform(
          node.nodeValue, 
          context
        );
